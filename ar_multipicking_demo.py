...
def ar_marker_CB(msg):
    try:
    	...
    	for marker_data in msg.markers:
            if marker_data.id == 7: # idの指定
            	listener.waitForTransform('/world', '/ar_marker_{0}'.format(marker_data.id), rospy.Time(), rospy.Duration(4.0))
            	(trans, rot) = listener.lookupTransform('/world', '/ar_marker_{0}'.format(marker_data.id), rospy.Time(0))
...
if __name__ == '__main__':
    ...
    pose_target_2.position.z = ar_pose.position.z + 0.06 # エンドエフェクタと床の衝突を防ぐために、0.06[m]の余裕を持たせている 
    ...
    pose_target_3 = Pose()
    pose_target_3.position.x = 0.03 # エンドエフェクタの位置(x,y,z)を指定
    pose_target_3.position.y = 0.0
    ...
    group.set_joint_value_target(pose_target_3, True) # 指定された位置姿勢をプランニング
    group.go() # プランニング結果を実行
    rospy.sleep(1.0)
    pose_current = group.get_current_pose()
    rospy.loginfo("Get Current Pose:\n{}\n".format(pose_current)) 
